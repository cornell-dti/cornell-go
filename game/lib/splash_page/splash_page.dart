      ...
      builder: (context, apiClient, child) {
        return TextButton(
          ...
          onPressed: () async {
            ...
            bool userExists = await apiClient.checkUserExists(idToken ?? "");
            if (userExists) {
              ...
              Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (context) =>
                      RegisterPageWidget(user: account, idToken: idToken),
                ),
              );
            }
        ...
        return TextButton(
          ...
          onPressed: () async {
            try {
              ...
              // Register user with Apple credentials and guest-level permissions
              // No Cornell email requirement - works like normal account creation
              final appleResult = await client.connectApple(
                  credential, "", LoginEnrollmentTypeDto.GUEST, "", "", "", []);

              if (appleResult == null) {
                displayToast(
                    "An error occurred while signing you up with Apple!",
                    Status.error);
              }
            } catch (e) {
              displayToast(
